...
// System posts that use a COMPOSED icon rather than a named one.
// Kept here for the same reason as SYSTEM_ICONS: so scripts/auditIcons.js can verify
// these compositions never collide with one another or with an educational topic.
export const SYSTEM_COMPOSITIONS = {
  verdictCorrect: { glyph: "crown", badge: "bCheck" },
  verdictWrong: { glyph: "crown", badge: "bCross" },
};

// Convenience lists for the audit script.
export const RESERVED_ICON_NAMES = Object.values(SYSTEM_ICONS);
export const RESERVED_COMPOSITION_KEYS = Object.values(SYSTEM_COMPOSITIONS).map(
  (c) => `${c.glyph}+${c.badge}`
);
