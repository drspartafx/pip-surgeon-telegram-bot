...
import { composeIcon } from "./composeIcon.js";
...
export function buildPosterSVG(title, iconKey) {
  // Accepts either a legacy named icon ("goldBar") or a composition
  // ({ glyph: "ship", badge: "bCrack" }) from the composable system.
  let iconMarkup;
  if (iconKey && typeof iconKey === "object") {
    iconMarkup = composeIcon(iconKey.glyph, iconKey.badge);
  } else {
